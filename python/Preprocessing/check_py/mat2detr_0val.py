...
# === VALIDATION FUNCTION ===
def validate_conversion(class_name="fly", sample_id="001"):
    print(f"\n🔍 Running validation for sample {class_name}{sample_id}...")

    json_path = os.path.join(datasets[class_name]["output_base"], "instances_train_original.json")
    ...
    if not matched_img:
        print(f"⚠️ Image {image_file} not found in JSON.")
        return
    ...
    print("\n🟢 .MAT original bounding boxes:")
    for box in boxes:
        print(box.tolist())

# === RUN CONVERSION AND VALIDATE ===
for class_name, paths in datasets.items():
    ...
    # Validate only for threshold 0 (original)
    validate_conversion(class_name=class_name, sample_id="001")
