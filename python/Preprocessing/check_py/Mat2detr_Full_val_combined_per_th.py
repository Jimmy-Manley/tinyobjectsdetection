...
def convert_mat_to_coco(threshold, suffix, input_img_dir, input_mat_dir, output_json_path, class_prefix):
    ...
    for mat_file in tqdm(os.listdir(input_mat_dir), desc=f"{class_prefix} @ Threshold {threshold}"):
        ...
        # Skip if image does not exist or mismatch in ID
...
def validate_conversion(class_name, suffix, sample_id="001"):
    try:
        ...
        if not matched_img:
            return f"⚠️ {class_name} {suffix}: {image_file} not in JSON — check if .mat matches correct folder."
        ...
        match_note = "✅" if len(matched_boxes) == boxes.shape[0] else "⚠️ Mismatch count"

        return f"{class_name} {suffix} {match_note} | JSON: {len(matched_boxes)} boxes, MAT: {boxes.shape[0]} boxes"
